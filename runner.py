...
data=Loader(datasize, N_points, kapa, time_series, shift)
...
for epoch in range(n_epochs):
  ...
  for batch,(x,velocity) in enumerate(data_loader):
    ...
    for i in range(1,x.shape[1]):
      ...
      loss = APloss_fn(score_model, x[:,i,:,:], x[:,i-1,:,:], kapa, velocity[:,i-1,:,:,:], time, mode)
      # print(loss)
      loss.backward()    
      optimizer.step()
    avg_loss += loss.item() * x.shape[0]
    num_items += x.shape[0]
  print('Average Loss: {:9f}'.format(avg_loss / num_items))
